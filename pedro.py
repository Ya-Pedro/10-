...
logging.basicConfig(filename='quiz.log', level=logging.INFO, format="%(asctime)s - %(message)s" , encoding='utf-8')
...
class QuizBot(BaseQuizBot): 
    def start_bot(self): 
        ...
        def handle_answer(call): 
            ...
            if call.data == correct_answer: 
                ...
                logging.info(f"Пользователь {username} ответил правильно на вопрос: {question}")  
            else: 
                self.bot.send_message(call.message.chat.id, f"Неправильно! Правильным ответом было: {correct_answer}")
                logging.info(f"Пользователь {username} ответил неправильно на вопрос: {question}")  
    ...
    def calculate_results(self, chat_id):
        ...
        if total_questions > 0:
            ...
            username = self.bot.get_chat(chat_id).username if self.bot.get_chat(chat_id).username else "Неизвестный пользователь"
            logging.info(f"Пользователь {username} ответил правильно на {correct_answers} из {total_questions} вопросов. Процент правильных ответов: {percentage:.2f}%")
        else:
